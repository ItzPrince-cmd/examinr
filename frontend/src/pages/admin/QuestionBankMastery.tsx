...
import { cn } from '../../lib/utils';
  ...
  interface Question { id: string;type: 'mcq' | 'short' | 'long' | 'numerical' | 'code' | 'image' | 'video';
  title: string;
  content: string;
  subject: string;
  topic: string;
  ...
  interface UploadFile { id: string;
  name: string;
  ...
  const QuestionBankMastery: React.FC = () => {
  ...
  const [topicHierarchy] = useState<TopicNode[]>([
    {
      id: '1',
      name: 'Physics',
      level: 0,
  ...
  const TreeNode: React.FC<{ node: TopicNode; depth: number }> = ({ node, depth }) => {
  ...
  return (<div className="relative w-32 h-32"><svg viewBox="0 0 100 100" className="transform -rotate-90"><circle cx="50" cy="50" r="40" stroke="currentColor" strokeWidth="8" fill="none" className="text-muted" /><motion.circle cx="50" cy="50" r="40" stroke="url(#qualityGradient)" strokeWidth="8" fill="none" strokeLinecap="round" initial={{ pathLength: 0 }} animate={{ pathLength: score / 100 }} transition={{ duration: 1, ease:"easeOut" }} style={{ strokeDasharray: `${2 * Math.PI * 40}`, strokeDashoffset: 0 }} />
  <defs><linearGradient id="qualityGradient" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" className={cn("text-green-500", getColor())} /><stop offset="100%" className={cn("text-emerald-500", getColor())} />
  </linearGradient>
  </defs>
  </svg><div className="absolute inset-0 flex flex-col items-center justify-center"><Brain className="h-6 w-6 text-muted-foreground mb-1" /><span className="text-2xl font-bold">
      {score}
    </span><span className="text-xs text-muted-foreground">Quality</span>
      </div>
    </div>
  );
};
    ...
    return (
      ...
      >
        ...
        >          {bulkEditMode && (
          ...
          )}<CardHeader className="pb-3"><div className="flex items-start justify-between"><div className="flex items-center gap-2"><div className={cn("p-2 rounded-lg",question.difficulty === 'easy' &&"bg-green-500/10",question.difficulty === 'medium' &&"bg-yellow-500/10",question.difficulty === 'hard' &&"bg-red-500/10",question.difficulty === 'expert' &&"bg-purple-500/10" )
}><TypeIcon className="h-4 w-4" />
</div>
<div><Badge variant="outline" className="text-xs">
      {question.subject} • {question.topic}
    </Badge>
</div></div><Badge variant={ question.status === 'published' ? 'default' : question.status === 'draft' ? 'secondary' : 'outline' } className="text-xs" >
      {question.status}
    </Badge>
</div>
</CardHeader><CardContent className="space-y-3"><div className="cursor-pointer" onClick={(e) => {
    e.stopPropagation();
    setIsFlipped(!isFlipped

    )
}} ><motion.div animate={{ rotateY: isFlipped ? 180 : 0 }} transition={{ duration: 0.6 }} style={{ transformStyle: 'preserve-3d' }} >
      {
  /* Front side */
}<div className={cn("min-h-[120px]", isFlipped &&"invisible" )}><p className="font-medium line-clamp-3">
      {question.title}
    </p><div className="flex flex-wrap gap-1 mt-3">{question.tags.slice(0, 3).map(tag => ( <Badge key={tag} variant="secondary" className="text-xs">
      {tag}</Badge> ))} {question.tags.length > 3 && ( <Badge variant="secondary" className="text-xs"> +{question.tags.length - 3}
    </Badge> )}
    </div>
</div>
      {/* Back side */} {isFlipped && ( <div className="absolute inset-0 p-4" style={{ transform: 'rotateY(180deg)' }}><div className="space-y-2"><div className="flex justify-between text-sm"><span className="text-muted-foreground">Usage</span>
  <span>
      {question.usageCount} times</span>
  </div><div className="flex justify-between text-sm"><span className="text-muted-foreground">Success Rate</span>
  <span>
      {question.successRate}%</span>
  </div><div className="flex justify-between text-sm"><span className="text-muted-foreground">Points</span>
  <span>
      {question.points}
    </span>
  </div><div className="flex justify-between text-sm"><span className="text-muted-foreground">Time</span>
  <span>
      ...
      {
  /* Stats Bar */
}<div className="grid grid-cols-3 gap-2 pt-3 border-t"><div className="text-center"><div className="text-xs text-muted-foreground">Quality</div><div className="flex items-center justify-center mt-1"><Progress value={question.qualityScore} className="w-12 h-2" />
</div>
</div><div className="text-center"><div className="text-xs text-muted-foreground">v{question.version}
    </div><Clock className="h-3 w-3 mx-auto mt-1 text-muted-foreground" />
</div><div className="text-center"><div className="text-xs text-muted-foreground">Difficulty</div><div className="flex items-center justify-center mt-1">{Array.from({ length: 4 }).map((_, i) => ( <div key={i} className={cn("w-1 h-3 mx-0.5 rounded-full",i < (question.difficulty === 'easy' ? 1 : question.difficulty === 'medium' ? 2 : question.difficulty === 'hard' ? 3 : 4) ?"bg-primary" :"bg-muted" )
} /> ))}
    </div>
</div>
</div>
      {
  /* Quick Actions */
}<div className="flex items-center gap-1 pt-2"><Button size="sm" variant="ghost" className="h-8 px-2"><Eye className="h-3 w-3" />
</Button><Button size="sm" variant="ghost" className="h-8 px-2"><Edit3 className="h-3 w-3" />
</Button><Button size="sm" variant="ghost" className="h-8 px-2"><Copy className="h-3 w-3" />
</Button><Button size="sm" variant="ghost" className="h-8 px-2 text-red-500"><Trash2 className="h-3 w-3" />
</Button>
</div>
        </CardContent>
        </Card>
...
</Badge> ) )}<Button variant="ghost" size="sm" onClick={() => {setSearchQuery(''

    );
      setFilters({subject: 'all',topic: 'all',difficulty: 'all',type: 'all',status: 'all'
      }
